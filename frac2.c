...
//	randqueue2(numc,idx,StarNum,sidx,all_star,eps);
	...
	for (i=0;i<n_sub;++i) {
		...
		for (j=0;j<nstar;++j) {
			ii = i*nstar + j;
			star_x[ii].m = x_sub[j].m;
			star_x[ii].x[0] = x_sub[j].x[0] + sub_centre_x[i].x[0];
			star_x[ii].x[1] = x_sub[j].x[1] + sub_centre_x[i].x[1];
			star_x[ii].x[2] = x_sub[j].x[2] + sub_centre_x[i].x[2];
			star_x[ii].x[3] = x_sub[j].x[3] + sub_centre_x[i].x[3];
			star_x[ii].x[4] = x_sub[j].x[4] + sub_centre_x[i].x[4];
			star_x[ii].x[5] = x_sub[j].x[5] + sub_centre_x[i].x[5];
		}
		fclose(fp);
	}
...
void make_king(int nstar, struct star *x, FILE *fp)
{
	...
	for (i=0;i<nstar;++i) {
		fscanf(fp,"%lf %lf %lf %lf %lf %lf %lf",\
				&x[i].m,&x[i].x[0],&x[i].x[1],&x[i].x[2],&x[i].x[3],&x[i].x[4],&x[i].x[5]);
	}
}
